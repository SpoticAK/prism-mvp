...
def main():
    ...
    if 'product_pointer' not in st.session_state:
    ...
    if st.session_state.sidebar_state == 'expanded':
        ...
        nav_pane, main_content = st.columns([1, 12], gap="small")
    ...
    with nav_pane:
        ...
        if st.session_state.sidebar_state == 'expanded':
            ...
            for category in categories.keys():
                ...
                # A bit of a hack to apply a class to a button
                st.markdown(f"""
                    <style>
                        div[data-testid="stHorizontalBlock"] button[kind="secondary"]:nth-child({list(categories.keys()).index(category) + 1}) {{
                            {'background-color: #E65C5F; color: #FFFFFF !important;' if is_active else ''}
                        }}
                    </style>
                """, unsafe_allow_html=True)
                if st.button(category, use_container_width=True, key=category, type="secondary"):
