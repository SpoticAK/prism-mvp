...
st.markdown("""
<style>
    html, body, [class*="st-"] { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji"; background-color: #FFFFFF; color: #212121; }
    ...
    [data-testid="stSidebar"] [data-testid="stImage"] > img { width: 60px !important; height: 60px !important; border-radius: 8px; object-fit: cover; }
    .sidebar-item-container { padding: 10px; border-radius: 10px; margin-bottom: 10px; background-color: #f9f9f9; }
...
def main():
    ...
    if 'app_loaded' not in st.session_state:
        ...
        # Shuffle indices for browsing experience, but saved items use original DataFrame indices (not shuffled positions)
        st.session_state.shuffled_indices = random.sample(list(df.index), len(df))

        try:
            query_params = st.query_params.to_dict()
            saved_from_url = [int(i) for i in query_params.get("saved", [])]
            # Persist saved products as original DataFrame indices (never as position in shuffled indices)
            st.session_state.saved_products = list(dict.fromkeys(saved_from_url))
            st.session_state.notes = {int(k.split('_')[1]): v for k, v in query_params.items() if k.startswith("note_")}
        except:
            st.session_state.saved_products, st.session_state.notes = [], {}

        # Set pointer by saved index using original DataFrame index, not shuffled mapping
        if st.session_state.saved_products:
            first_saved_index = st.session_state.saved_products
    ...
    current_product = df.loc[current_index]
